...
// test72r1 - Synthesise our own FIFO inline
// test72r2 - Generate our own FIFO as a separate block and manually replace with hardened one.
...
public class KiwiFifo<DT>
{
  ...
  public KiwiFifo(int capacity) // Constructor
  {
    entries = 0;
    in_ptr = 0;
    out_ptr = 0;
    ...
    store = new DT [capacity];
  }
  
  public void Queue(DT arg)
  {
    if (entries < capacity)
      lock(this)
        {
...
class Test72r1
{
  ...
  {
    KiwiFifo<int> the_fifo = new KiwiFifo<int>(100 );
    Kiwi.Pause();
  ...
  public static void Main()
     {
       Kiwi.Pause();
       Console.WriteLine("Start NativeFifo 72r1 Test");
       int rc = NativeFifoTest();
       Kiwi.Pause();
       Console.WriteLine("NativeFifo 72r1 Test: End of Demo. rc={0}", rc);      
       Kiwi.Pause();
